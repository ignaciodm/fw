...
describe ProjectQueryCommandLine do
  it 'should accept -s option' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,SHOT,VERSION')).to eq([
                                                                           'the hobbit,01,64',
                                                                           'lotr,03,16',
                                                                           'the hobbit,40,32',
                                                                           'king kong,42,128'
  ...
  it 'should accept -s option and -o together' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,SHOT,VERSION,STATUS -o FINISH_DATE,INTERNAL_BID')).to eq([
                                                                                                              'lotr,03,16,finished',
  ...
  it 'should accept -s option and -f together' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,SHOT,VERSION,STATUS -f FINISH_DATE=2006-07-22')).to eq(['king kong,42,128,not required'])
  end

  it 'should accept sum and collect option and -g together' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,INTERNAL_BID:sum,SHOT:collect -g PROJECT')).to eq([
  ...
  it 'should accept sum and collect option and -g together' do
    ...
    expect(ProjectQueryCommandLine.run('-s PROJECT,INTERNAL_BID:sum,SHOT:collect -g PROJECT')).to eq(expected)
  end

  it 'should accept min and max and -g together' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,INTERNAL_BID:min,SHOT:max -g PROJECT')).to eq([
                                                                                                   'the hobbit,22.8,40',
                                                                                                   'lotr,15.0,03',
                                                                                                   'king kong,30.0,42'
                                                                                                 ])
  ...
  it 'should accept count and -g together' do
    expect(ProjectQueryCommandLine.run('-s PROJECT,SHOT:count -g PROJECT')).to eq([
                                                                                    'the hobbit,2',
                                                                                    'lotr,1',
                                                                                    'king kong,1'
                                                                                  ])
  ...
  it 'should order by finish date' do
    expect(ProjectQueryCommandLine.run('-o FINISH_DATE')).to eq([
